...
class SVM(SKLearnEstimator):
    ...
    def search_space(cls,data_size,task):
        space={
            ...
            'kernel':{'domain':tune.choice(['linear','poly','sigmoid','rbf']),'init_value':'rbf'},
...
class GPC(SKLearnEstimator):
    ...
    def search_space(cls,data_size,task):
        space={
            'kernel':{'domain':tune.choice([1.0*RBF(),1.0*DotProduct(),1.0*Matern(),1.0*RationalQuadratic,1.0*WhiteKernel()]),'init_value':1.0*RBF() }
            #'kernel':{'domain':tune.choice(['rbf','dot','matern','rq','wh']),'init_value':'rbf' },
            #'kernel':{'domain':tune.choice(['rbf','dot','matern','rq','wh']),'init_value':'rbf' },
            #'length_scale':{'domain':tune.uniform(1e-4,100),'init_value':1},
            #'sigma_0':{'domain':tune.uniform(1e-4,100),'init_value':1},
            #'alpha':{'domain':tune.uniform(1e-4,100),'init_value':1},
            #'noise_level':{'domain':tune.uniform(1e-4,100),'init_value':1}
        }
        return space
    ...
    def _fit(self, X_train, y_train, **kwargs):
            ...
            try:
                if(self.params['kernel']=='rbf'):
                    params={'kernel':1.0*RBF(),'n_jobs':-1}
                    model = self.estimator_class(**params)
                elif(self.params['kernel']=='dot'):
                    params={'kernel':1.0*DotProduct(),'n_jobs':-1}
                    model = self.estimator_class(**params)
                elif(self.params['kernel']=='matern'):
                    params={'kernel':1.0*Matern(),'n_jobs':-1}
                    model = self.estimator_class(**params)
                elif(self.params['kernel']=='wh'):
                    params={'kernel':1.0*WhiteKernel(),'n_jobs':-1}
                    model = self.estimator_class(**params)
                elif(self.params['kernel']=='rq'):
                    params={'kernel':1.0*RationalQuadratic(),'n_jobs':-1}
                    model = self.estimator_class(**params)
            except KeyError:
                    print('KeyError , use RBF',flush=True)
                    params={'kernel':1.0*RBF(),'n_jobs':-1}
                    model = self.estimator_class(**params)
            if logger.level == logging.DEBUG:
                # xgboost 1.6 doesn't display all the params in the model str
...
class DTC(SKLearnEstimator):
    ...
    def search_space(cls,data_size,task):
        space={
            ...
            'max_depth':{'domain':tune.randint(2,100),'init_value':None},
            'min_samples_split':{'domain':tune.randint(2,1000),'init_value':2},
            'min_samples_leaf':{'domain':tune.randint(1,1000),'init_value':1},
            'max_features':{'domain':tune.choice(['auto','sqrt','log2']),'init_value':None}
        }
        return space
...
class ExTC(SKLearnEstimator):
    def __init__(self,task='classification',**config):
        super().__init__(task,**config)
        from sklearn.tree import ExtraTreeClassifier
        self.estimator_class=ExtraTreeClassifier
    ...
    def search_space(cls,data_size,task):
        space={
            ...
            'min_samples_leaf':{'domain':tune.randint(1,1000),'init_value':1},
            'max_features':{'domain':tune.choice(['auto','sqrt','log2']),'init_value':None},
            'min_impurity_decrease':{'domain':tune.uniform(0,20),'init_value':0}
        }
        return space
...
class LR(SKLearnEstimator):
    ...
    def search_space(cls,data_size,task):
        space={
                 'penalty':{'domain':tune.choice(['l1','l2','elasticnet','none']),'init_value':'l2'},
                 'solver':{'domain':tune.choice(['saga']),'init_value':'saga'},
                 'l1_ratio':{'domain':tune.uniform(0,1),'init_value':None},
                 'C':{'domain':tune.uniform(0.001,10),'init_value':1},
            }
        return space
